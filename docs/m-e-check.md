...
|----|------|
| 機器ID | 機器番号（機器表側のID） |
| ID照合 | 機器ID照合を `○ / ×` で表示（`要確認` が来た場合はそのまま表示） |
| 図面番号 > 電気図 | 盤表側の図面番号 |
| 図面番号 > 機械図 | 機器表側の図面番号 |
| 容量（KW） > 電気図 | 盤表側の容量 |
| 容量（KW） > 機械図 | 機器表の判定採用容量（空欄時は機器表消費電力） |
| 容量（KW） > 差分 | `盤表 - 機器表` の容量差 |
| 台数 > 電気図 | 盤表側の台数 |
| 台数 > 機械図 | 機器表側の台数 |
| 台数 > 差分 | `盤表 - 機器表` の台数差 |
...
| 列 | 説明 |
|----|------|
| （画面表示以外の内部列を含む） | 総合判定・判定理由・各判定（名称判定/台数判定/容量判定/機器ID照合）・記載名・図面番号・盤表 記載トレース など |
