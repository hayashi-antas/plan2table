...
1. `panel_file`（電気図PDF） -> Raster抽出（`_run_raster_job`）
2. `equipment_file`（機械図PDF） -> Vector抽出（`_run_vector_job`）
...
- Vector（機械図PDF）: `機器番号 / 名称 / 動力(50Hz)_消費電力(KW) / 台数 / 図面番号`
- Raster（電気図PDF）: `機器番号 / 機器名称 / 電圧(V) / 容量(kW) / 図面番号`
...
## 3. Vector側（機械図PDF）の認識

### Q. 機械図PDFで「対象の表」をどう見つけていますか？
...
## 4. Raster側（電気図PDF）の認識

### Q. 電気図PDFで「どこが表か」をどう見つけていますか？
...
### Q. ID照合は `○/×` 以外にもなりますか？
A. なります。表示列 `ID照合` は `機器ID照合` を基に `○ / × / 要確認` を表示します。  
総合判定が `要確認` の行は、ID照合も `要確認` で表示されます。  
また、画面上部の `ID照合一致` は `機器ID照合 = ◯` の件数のみをカウントします（`要確認` は含めない）。
...
### Q. 電気図側に同一機器IDが複数行あるときは？
A. Raster側でIDごとに集約し、件数・名称候補・容量候補・図面番号候補を保持して判定します。  
図面/名称/容量が複数パターンある場合は `電気図 記載トレース` 列に残します（`_format_trace_rows`）。

### Q. 電気図で機器IDが空欄の行はどう扱いますか？
A. Rasterの「IDなし集約」（`raster_missing_id_agg`）に入り、Unifiedでは `判定理由=電気図ID未記載` の `要確認` 行として出力します。
