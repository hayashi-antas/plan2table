...
表ラベルからブロック化できない枠でも、タイトル候補が確認できる場合があります。  
`build_frame_rows_from_segments()` では未使用タイトル候補を再走査し、次を満たすものを補完行として追加します。

- タイトル語として妥当（機器系語を含み、参考図・通線孔・施工注記などの除外語に該当しない）
- 既存ブロックに割り当て済みではない
- 番号は `_pick_code_for_anchor()` の厳しめ閾値で近傍候補のみ採用（誤付与を抑制）

これにより、ページ全体で一部枠だけ表抽出が失敗しても、`タイトル,番号` 形式の行として回収できます。
...
- `r"^質最" -> "質量"`
  - 用途: `質量` ラベル先頭2文字のOCR崩れ補正
  - 実例: `質最約570g` を `質量約570g` として扱い、`質量,約570g` を復元
...
- `質★15kg`
  ...
  - `r"質[★＊*]+"` 補正で `質量` ラベルとして抽出
- `質最約570g`
  - Vision OCRで `質量` の `量` が `最` 化
  - `r"^質最"` 補正で `質量,約570g` として抽出
