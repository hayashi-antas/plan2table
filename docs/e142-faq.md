...
### Q. `マグネットセンサー（露出型）取付参考例` はどう出ますか？
A. タイトルのみです。番号・表ペアは出力しません。
...
### Q. 表ラベルは何を見ていますか？
A. `電源電圧, 入力電圧, 出力電圧, 消費電流, 消費電力, 質量, 材質, 形状, 色調, 塗色, 塗装, 備考` です。
...
### Q. HTML結果は表ですか？
A. いいえ。CSV各行をそのまま `<ol><li>` で1行表示します。
