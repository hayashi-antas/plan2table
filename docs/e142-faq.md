...
### Q. 表がない枠は？
A. はい。該当枠だけ `タイトル,番号` で補完出力します（他枠に表ありでも同様）。
...
### Q. 表ラベルは何を見ていますか？
A. 基本は `電源電圧, 入力電圧, 出力電圧, 出力電流, 消費電流, 消費電力, 質量, 材質, 形状, 色調, 塗色, 塗装, 備考` です。加えて `カメラ1/4型...` のような行頭 `カメラ` 行は補助規則で抽出します。
...
### Q. OCRの誤認識はそのままCSVに出ますか？
A. 一部は後処理で補正します。例えば `質量` 行で `9q` と読まれた場合は `9g` に補正し、`マグネット9g` のようにコロンが欠落した場合は `マグネット:9g` に補正します。ラベル側では `質最約570g` のようなOCR崩れも `質量,約570g` として復元します。
