...
> **注**: 現時点では、**決まった形式のPDF**（機器図は上部に横並び2表、または要約左表があるページ構成。電気図は所定の見出し語群を含む表レイアウト）を前提としており、それ以外の形式では正しく動作しない場合があります。
...
- [5. PDF から CSV への抽出](#pdf-to-csv)
  - [5.1 電気図PDF → Raster CSV（raster_extractor）](#raster-extractor)
    - [5.1.1 処理の流れ](#raster-flow)
    - [5.1.2 依存関係](#raster-deps)
    - [5.1.3 Vision API の結果の整理（表にするまで）](#raster-vision)
  - [5.2 機器図PDF → Vector CSV（vector_extractor）](#vector-extractor)
...
- [6. 統合処理（Unified CSV）](#unified)
  ...
  - [6.5 出力列の存在理由（設計意図）](#unified-columns-why)
  - [6.6 実データ検証メモ（2026-02-19）](#unified-validation-20260219)
...
以下では、**機器図PDFを Vector 抽出・電気図PDFを Raster 抽出** する前提で用語を説明しています。これは、現状の実装で利用しているサンプルが「機器図＝ベクトルPDF・電気図＝ラスターPDF」であるためであり、仮の対応です。別形式のPDFを扱う場合は別の抽出パスが必要になります。

| 用語 | 説明 |
|------|------|
| **機器図** | 換気機器図など、機器番号・名称・消費電力・台数などを記載した表。**現状の想定**ではPDFはベクトル（テキスト/表線）で構成される。 |
| **電気図** | 電力制御盤の機器一覧。機器番号・機器名称・電圧・容量(kW)などを記載。**現状の想定**ではPDFはラスター（スキャン画像）として扱う。 |
| **Vector** | 機器図PDFから **pdfplumber**[^1] で表構造を検出し抽出したCSV。内部では「vector」ジョブとして扱う。 |
| **Raster** | 電気図PDFを **pdftoppm**[^3] と **Pillow**[^4] で画像化し、**Google Cloud Vision API**[^2] でOCRして、4列表＋図面番号として抽出したCSV。内部では「raster」ジョブとして扱う。 |
...
- **入力**
  - **機器図PDF**（[equipment_file](../main.py#L1019)）: 換気機器図など、表形式のPDF。実装は全ページを走査し、対象ページごとに「上部2表」または「要約左表（フォールバック）」を抽出する。
  - **電気図PDF**（[panel_file](../main.py#L1018)）: 電力制御電気図。実装は `extract_raster_pdf(page=0)` で全ページを順に画像化してOCRする。
- **出力**
  - **HTML**: 総合判定の主要列（総合判定・判定理由・機器ID・機器図 記載名・台数差・容量差など）を簡易表示。
  - **CSV**（`unified.csv`）: 統合結果の22列（[OUTPUT_COLUMNS](../extractors/unified_csv.py)）。ダウンロード用エンドポイント `GET /jobs/{job_id}/unified.csv` で取得。文字コードは **UTF-8 with BOM (`utf-8-sig`)**。
...
1. 画面上で **機器図PDF** と **電気図PDF** の2つのファイルを選択（ドラッグ＆ドロップまたはファイル選択）。
...
[main.py](../main.py) の `CUSTOMER_DISPLAY_SINGLE_COLUMNS` と `CUSTOMER_DISPLAY_GROUP_COLUMNS` で定義されている。  
表示は2段ヘッダー（単独列 + グループ列）で、unified CSV の列名対応は以下のとおり。

| 表示ラベル | 対応する unified CSV の列（候補のいずれか） |
|------------|---------------------------------------------|
| 機器ID | 機器ID、機器番号、機械番号 |
| ID照合 | 機器ID照合（表示時に `○ / × / 要確認` へ変換。総合判定が `要確認` の行は `要確認` を表示） |
| 図面番号 > 電気図 | 電気図 図面番号、図面番号、図番 |
| 図面番号 > 機械図 | 機器図 図面番号、機器図図面番号 |
| 容量（KW） > 電気図 | 電気図 容量(kW)、電気図 容量合計(kW)、raster_容量(kW)_sum |
| 容量（KW） > 機械図 | 機器図 判定採用容量(kW)、機器図 消費電力(kW)、機器図 容量合計(kW)、vector_容量(kW)_calc |
| 容量（KW） > 差分 | 容量差(kW)、容量差分(kW) |
| 台数 > 電気図 | 電気図 台数、raster_match_count、raster_台数_calc |
| 台数 > 機械図 | 機器図 台数、台数、vector_台数_numeric |
| 台数 > 差分 | 台数差、台数差（電気図-機器図）、台数差分 |

※ 表の下に `台数差 / 容量差は 電気図 - 機器図` の注記を表示する。
...
- `機器図記載`: vector 抽出（`vector.csv`）の行数
- `電気図記載`: raster 抽出（`raster.csv`）の行数
- `ID照合一致`: `機器ID照合 = ◯` の行数（`要確認` は含めない）
- `不一致`: `総合判定 = ✗` の行数
- `要確認`: `総合判定 = 要確認` の行数

※ `機器図記載` / `電気図記載` は、統合後（unified）の集約行数ではなく、抽出段階の生行数を使う。
...
1. **Panel → Raster**  
   [panel_file](../main.py#L1018) のバイト列を [_run_raster_job](../main.py#L771) に渡す。`page=0`（全ページ対象）で電気図PDFを画像化し、Vision API でOCRして **raster.csv** を生成し、raster ジョブとして保存する。
2. **Equipment → Vector**  
   [equipment_file](../main.py#L1019) のバイト列を [_run_vector_job](../main.py#L804) に渡す。機器図PDFから pdfplumber で表を抽出し、**vector.csv**（5列）を生成し、vector ジョブとして保存する。
3. **Unified**  
...
### 5.1 電気図PDF → Raster CSV（raster_extractor）

**モジュール**: [extractors.raster_extractor](../extractors/raster_extractor.py)

電気図PDFは「画像として扱う」前提である。スキャンされたPDFや、テキストが画像として埋め込まれているPDFを想定している。
...
### 5.2 機器図PDF → Vector CSV（vector_extractor）

**モジュール**: [extractors.vector_extractor](../extractors/vector_extractor.py)

機器図PDFは **ベクトル（テキスト・線）** で構成されている前提である。表の罫線やセルが PDF の描画命令として存在し、pdfplumber で表として検出できる形式を想定している。
...
4. **ヘッダーの復元**  
   表領域内の単語から、1行目データより上をヘッダーとみなし、グループ行・サブ行・単位行を再構成する（[reconstruct_headers_from_pdf](../extractors/vector_extractor.py#L646)）。「換気機器図」などのタイトルや、最初の機器番号の位置からデータ開始位置を決め、その上をヘッダーとして扱う。
5. **データ行の抽出**  
...
| vector_count | 台数 |
| vector_drawing_number | 図面番号, 図番, 機器図 図面番号 |
| raster_name | 機器名称, 名称 |
...
- マッチした行数: `raster_match_count`（＝電気図にその機器番号が何行あるか）
- 容量(kW): 非空・出現順・重複除去で容量候補を保持する（数値化できる値は正規化した表示値にする）。
- 名称候補: 非空・出現順・重複除去で連結し、`電気図 記載名` に出力する。
- 図面番号: 非空・出現順・重複除去で連結し、`電気図 図面番号` に出力する（例: `E-024,E-031`）。
...
- **raster のみ機器**: vector に存在しない機器番号が raster にある場合、その行は統合結果の末尾に追加し、`総合判定=✗`、`判定理由=機器図に記載なし` として出力する。
- **raster の機器ID空欄行**: raster に機器IDが空欄の行がある場合は、照合対象には入れず、名称・電圧・容量・図面番号（同一内容は件数集約）ごとに統合結果の末尾へ追加する。`総合判定=要確認`、`判定理由=電気図ID未記載` を設定する。
- **機器図 記載名**: unified の `機器図 記載名` は vector 側の名称列（`vector_name`）を NFKC + 空白除去した値を採用する。
- **機器図 図面番号**: vector 側の図面番号を機器番号単位で集約し、非空・出現順・重複除去で連結して出力する。
- **存在判定（境界）**: 片側に存在しないことが確定した場合は `mismatch`（表示 `✗`）。`電気図ID未記載` など紐付け不能で存在判定自体が確定できない場合は `review`（表示 `要確認`）。
- **台数**: vector の「台数」と raster のマッチ行数を比較し、差分は **台数差** として出力する。
- **台数判定**: `台数判定` は `◯` / `✗` / `要確認` の3値を出力する。比較不能（例: 機器図台数欠損）は `要確認`。
- **容量**: `機器図 消費電力(kW)` には vector の元値（raw）を保持し、`電気図 容量(kW)` は raster 側容量候補を非空・出現順・重複除去でカンマ連結した値を出力する。  
  判定用には `機器図 判定モード` と `機器図 判定採用容量(kW)` を別列で出力し、モード付き容量の根拠は `容量判定補足` に記録する。  
  `容量差(kW)` は **両側が単一数値のときのみ** `電気図 - 機器図` を出力する。  
  **容量判定**: 非数値（例: `10VA`）、カンマ区切り複数候補（例: `0.75,1.5`）、同一IDで複数容量がある場合は `要確認`。  
  ただし機器図容量が `(冷)/(暖)/(低温)` の形式で、機器名称に `冷房専用` / `暖房専用` / `低温専用` がある場合は、対応モードの値を採用して数値比較する。  
...
- **電気図 記載トレース**: 同一機器IDで図面番号・名称・容量の組み合わせが2種類以上ある場合、raster 行単位の `図面:<番号> 名称:<名称> 容量:<値>` を ` || ` 区切りで出力する。  
...
- **判定理由（一本化）**: 旧 `不一致内容/確認理由` 列は廃止。CSVには `判定理由` 1列のみ出力する。  
  1) 総合判定 `◯` の場合は空欄  
  2) 総合判定 `✗` / `要確認` の場合は主原因1文（例: `台数差分=1`, `容量が複数候補`, `電気図ID未記載`）  
...
unified CSV は **vector の生データ列は含めず**、次の `OUTPUT_COLUMNS` の22列だけを出力する。CSVは **UTF-8 with BOM (`utf-8-sig`)** で出力する。
...
```csv
総合判定,台数判定,容量判定,名称判定,機器ID照合,判定理由,機器ID,機器図 記載名,電気図 記載名,機器図 台数,電気図 台数,台数差,機器図 消費電力(kW),機器図 モード容量(kW),機器図 判定モード,機器図 判定採用容量(kW),容量判定補足,電気図 容量(kW),容量差(kW),機器図 図面番号,電気図 図面番号,電気図 記載トレース
...
<a id="unified-columns-why"></a>
### 6.5 出力列の存在理由（設計意図）

ここでは `OUTPUT_COLUMNS` の 22 列それぞれについて、「なぜその列を持つか」を明示する。

| 列名 | 存在理由（なぜ必要か） |
|------|-------------------------|
| 総合判定 | 台数・容量・名称・存在判定を統合した最終結論を 1 列で示すため。運用上はまずこの列で優先確認対象を絞る。 |
| 台数判定 | 台数だけの成否を独立表示し、原因切り分けを容易にするため。 |
| 容量判定 | 容量だけの成否を独立表示し、モード容量起因の問題を可視化するため。 |
| 名称判定 | 名称だけの成否を独立表示し、機器同定のズレを分離して扱うため。 |
| 機器ID照合 | 機器図IDが電気図側にも存在するかを独立表示し、ID存在不一致を即時確認できるようにするため。 |
| 判定理由 | `✗` / `要確認` の主原因を 1 文で示し、一次トリアージを高速化するため。 |
| 機器ID | 照合の主キーであり、再調査時に図面・原票へ戻る起点となるため。 |
| 機器図 記載名 | 基準側（vector）の名称を保持し、比較元を固定するため。 |
| 電気図 記載名 | 照合先（raster）の名称候補を保持し、名称不一致の実体を見せるため。 |
| 機器図 台数 | 台数比較の基準値（expected）を残すため。 |
| 電気図 台数 | 台数比較の実績値（actual）を残すため。 |
| 台数差 | `電気図 - 機器図` の差分を定量表示し、ずれの大きさを即時判断できるようにするため。 |
| 機器図 消費電力(kW) | 機器図の raw 値を保持し、判定前の元データを失わないため。 |
| 機器図 モード容量(kW) | `(冷)/(暖)/(低温)` の抽出結果を保存し、「候補として何が存在したか」を監査可能にするため。削ると判定根拠の説明力が下がる。 |
| 機器図 判定モード | 実際に採用した比較モード（例: `冷`, `最大値(低温)`, `未確定`）を明示し、モード選択ロジックの結果を追跡するため。 |
| 機器図 判定採用容量(kW) | 最終比較に使った単一数値を保持し、`容量差(kW)` の計算根拠を再現できるようにするため。 |
| 容量判定補足 | モード採用理由（名称ヒント、最大値フォールバック、未確定理由など）を日本語で残し、報告時の説明責任を満たすため。 |
| 電気図 容量(kW) | 電気図側の容量候補を保持し、複数候補や表記ゆれを隠さないため。 |
| 容量差(kW) | `電気図 - 機器図` の差分。両側が単一数値のときのみ算出し、比較不能ケースの誤読を防ぐため。 |
| 機器図 図面番号 | 機器図側の出典ページを保持し、根拠追跡を可能にするため。 |
| 電気図 図面番号 | 電気図側の出典ページを保持し、同一機器IDが複数図面にまたがるケースを追えるようにするため。 |
| 電気図 記載トレース | 同一機器ID内の `図面・名称・容量` の組み合わせを保持し、部分一致による見落としを防ぐため。内訳差異の検出で最重要。 |

**特に意図が見えにくいが重要な列**

- `機器図 モード容量(kW)` / `機器図 判定モード` / `機器図 判定採用容量(kW)` / `容量判定補足` は、モード付き容量判定の再現性と説明責任を担う。
- `電気図 記載トレース` は、同一機器IDでの複数記載（図面違い・名称違い・容量違い）を潰さず残すための監査列である。

---

<a id="unified-validation-20260219"></a>
### 6.6 実データ検証メモ（2026-02-19）

- 検証対象: `me-check_照合結果_20260219_0551.csv`（189行・22列）。
- `機器図 消費電力(kW)` が複合表記（`/` を含む）な行では、`機器図 モード容量(kW)` に `冷=... , 暖=... , 低温=...` 形式でパース結果が残る。
- 判定フローは次の順で確認できる。  
...
- `機器図 モード容量(kW)` は判定の説明責任を担う検証列として有効。提出用表示で見づらい場合は、削除よりも「右端へ寄せる / UIで非表示」を推奨する。
...
| POST | `/customer/run` | 機器図PDF・電気図PDFを multipart で受け取り、raster / vector 抽出（既定は並列、`ME_CHECK_PARALLEL_EXTRACT=0` で直列）の後に unified を実行し、結果をHTML（簡易表 or エラー）で返す。 |
| GET | `/jobs/{job_id}/unified.csv` | 指定した unified ジョブの `unified.csv` をダウンロードする（ダウンロード名は `me-check_照合結果_YYYYMMDD_HHMM.csv`）。 |

`/customer/run` のパラメータは `panel_file`（電気図PDF）と `equipment_file`（機器図PDF）の2つが必須。  
...
- **Raster**: 電気図PDFを1本だけアップロードし、`POST /raster/upload` で raster.csv を生成。Job ID と CSV ダウンロードリンクが返る。
- **Vector**: 機器図PDFを1本だけアップロードし、`POST /vector/upload` で vector.csv を生成。同様に Job ID とダウンロードリンク。
