...
- **raster の機器ID空欄行**: raster に機器IDが空欄の行がある場合は、照合対象には入れず、名称・容量・図面番号（同一内容は件数集約）ごとに統合結果の末尾へ追加する。`照合結果=要確認`、`不一致内容=機器ID未記載（盤表）` を設定する。
...
- **不一致内容**: 照合が「不一致」のとき、次の優先順で **1つだけ** 設定する。  
  ...
  ※ raster の機器ID空欄行として追加された行では `機器ID未記載（盤表）` を設定する（`照合結果=要確認`）。
