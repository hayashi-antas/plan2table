...
|------------|---------------------------------------------|
| 機器ID | 機器ID、機器番号、機械番号 |
| ID照合 | 機器ID照合（表示時に `○ / ×` へ変換） |
| 図面番号 > 電気図 | 盤表 図面番号、図面番号、図番 |
| 図面番号 > 機械図 | 機器表 図面番号、機器表図面番号 |
| 容量（KW） > 電気図 | 盤表 容量(kW)、盤表 容量合計(kW)、raster_容量(kW)_sum |
| 容量（KW） > 機械図 | 機器表 判定採用容量(kW)、機器表 消費電力(kW)、機器表 容量合計(kW)、vector_容量(kW)_calc |
| 容量（KW） > 差分 | 容量差(kW)、容量差分(kW) |
| 台数 > 電気図 | 盤表 台数、raster_match_count、raster_台数_calc |
| 台数 > 機械図 | 機器表 台数、台数、vector_台数_numeric |
| 台数 > 差分 | 台数差、台数差（盤表-機器表）、台数差分 |
...
- **raster の機器ID空欄行**: raster に機器IDが空欄の行がある場合は、照合対象には入れず、名称・電圧・容量・図面番号（同一内容は件数集約）ごとに統合結果の末尾へ追加する。`総合判定=要確認`、`判定理由=盤表ID未記載` を設定する。
...
```csv
総合判定,台数判定,容量判定,名称判定,機器ID照合,判定理由,機器ID,機器表 記載名,盤表 記載名,機器表 台数,盤表 台数,台数差,機器表 消費電力(kW),機器表 モード容量(kW),機器表 判定モード,機器表 判定採用容量(kW),容量判定補足,盤表 容量(kW),容量差(kW),機器表 図面番号,盤表 図面番号,盤表 記載トレース
...
| 機器ID照合 | 機器表IDが盤表側にも存在するかを独立表示し、ID存在不一致を即時確認できるようにするため。 |
| 判定理由 | `✗` / `要確認` の主原因を 1 文で示し、一次トリアージを高速化するため。 |
| 機器ID | 照合の主キーであり、再調査時に図面・原票へ戻る起点となるため。 |
| 機器表 記載名 | 基準側（vector）の名称を保持し、比較元を固定するため。 |
| 盤表 記載名 | 照合先（raster）の名称候補を保持し、名称不一致の実体を見せるため。 |
| 機器表 台数 | 台数比較の基準値（expected）を残すため。 |
| 盤表 台数 | 台数比較の実績値（actual）を残すため。 |
| 台数差 | `盤表 - 機器表` の差分を定量表示し、ずれの大きさを即時判断できるようにするため。 |
| 機器表 消費電力(kW) | 機器表の raw 値を保持し、判定前の元データを失わないため。 |
| 機器表 モード容量(kW) | `(冷)/(暖)/(低温)` の抽出結果を保存し、「候補として何が存在したか」を監査可能にするため。削ると判定根拠の説明力が下がる。 |
| 機器表 判定モード | 実際に採用した比較モード（例: `冷`, `最大値(低温)`, `未確定`）を明示し、モード選択ロジックの結果を追跡するため。 |
| 機器表 判定採用容量(kW) | 最終比較に使った単一数値を保持し、`容量差(kW)` の計算根拠を再現できるようにするため。 |
| 容量判定補足 | モード採用理由（名称ヒント、最大値フォールバック、未確定理由など）を日本語で残し、報告時の説明責任を満たすため。 |
| 盤表 容量(kW) | 盤表側の容量候補を保持し、複数候補や表記ゆれを隠さないため。 |
| 容量差(kW) | `盤表 - 機器表` の差分。両側が単一数値のときのみ算出し、比較不能ケースの誤読を防ぐため。 |
| 機器表 図面番号 | 機器表側の出典ページを保持し、根拠追跡を可能にするため。 |
| 盤表 図面番号 | 盤表側の出典ページを保持し、同一機器IDが複数図面にまたがるケースを追えるようにするため。 |
| 盤表 記載トレース | 同一機器ID内の `図面・名称・容量` の組み合わせを保持し、部分一致による見落としを防ぐため。内訳差異の検出で最重要。 |

**特に意図が見えにくいが重要な列**

- `機器表 モード容量(kW)` / `機器表 判定モード` / `機器表 判定採用容量(kW)` / `容量判定補足` は、モード付き容量判定の再現性と説明責任を担う。
- `盤表 記載トレース` は、同一機器IDでの複数記載（図面違い・名称違い・容量違い）を潰さず残すための監査列である。
